...
VOID AddFlags(frame &f, ValSpecRec &v)
{
    uint64_t flags = 0;
    size_t numbytes = GetByteSize(v.type);
    
    for (size_t i = 0; i < numbytes; i++)
    {
        flags <<= 8;
        flags |= (uint64_t) (((unsigned char*) &(v.value))[numbytes-i-1]);
    }

    const char* flag_name[17] = { "R_CF",   "R_PF",   "R_AF",  "R_ZF", 
                                  "R_SF",   "R_TF",   "R_IF",  "R_DF", 
                                  "R_OF",   "R_IOPL", "R_NT",  "R_RF", 
                                  "R_VM",   "R_AC",   "R_VIF", "R_VIP", 
                                  "R_ID" };

    size_t flag_index[17]     = {  0,  2,  4,  6, 
                                   7,  8,  9, 10, 
                                  11, 12, 14, 16, 
                                  17, 18, 19, 20, 
                                  21 };
    ...
    {
        operand_info *o = f.mutable_std_frame()->mutable_operand_pre_list()->add_elem();
        ...
        o->mutable_operand_info_specific()->mutable_reg_operand()->set_name(flag_name[i]);

        uint64_t mask = 1 << flag_index[i];
...
VOID FlushInstructions()
{
    ...
    for(uint32_t i = 0; i < g_bufidx; i++) {
        ...
        /* The operand_list is a required field, so we must access it
           even if there are no operands or protobuffers will complain
           to us. */
        ...
        for (uint32_t j = 0; j < g_buffer[i].values_count; j++) {
            ...
            if (tracker->isReg(v.type)) {
                string t = pin_register_name((REG)v.loc);
                if (t == "R_RFLAGS" || t == "R_EFLAGS") {
                    AddFlags(fnew,v);
                    continue;
                }
            }
